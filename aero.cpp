...
long long int binaria(long long int valor, long long int izq, long long int der)
{
	long long int mid = ceil(izq + (der - izq) / 2);
	if(izq > der)
		return mid;
	if(divis(mid) < valor)
		binaria(valor, mid + 1 , der);
	else
		binaria(valor, izq, mid - 1);
}


/*
long long int binaria(long long int valor)
{
...
void merge(long long int ini, long long int mid, long long int fin)
{
	...
	for(i = 0 ; i < tam1 ; i++)
		izq[i] = personas[ini + i];
	for(j = 0 ; j < tam2 ; j++)
		der[j]  = personas[mid + 1 + j];
	i = 0;
	j = 0;
	k = ini;
	while(i < tam1 && j < tam2)
...
int main(void)
{
	...
	for(int i = 0 ; i < peeps ; i++)
	{
		...
		mergeSort(0, personas.size() - 1);
		cout << binaria(value, 0, 1000000000000000000) << "\n";
	return 0;
}
